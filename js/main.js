(function () {
  ...
  const assetVersion = "20260522-model-label";
  ...
  function productCard(product) {
    const label = categoryLabel(product.category);
    const detailHref = `product-detail.html?id=${encodeURIComponent(product.code)}&v=20260522-model-label`;
    return `
  ...
  function renderProductDetail() {
    ...
    if (!product) {
      mount.innerHTML = `<div class="page-hero"><h1>PRODUCT NOT FOUND</h1><p>제품 데이터를 찾을 수 없습니다.</p><a class="btn btn-dark" href="products.html?v=20260522-model-label">제품 목록으로 돌아가기</a></div>`;
      return;
    }
